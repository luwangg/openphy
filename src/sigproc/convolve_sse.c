		    ...
		    float *out)
{
	...
	const float complex *_out = (const float complex *) out;
	...
	if (conv_func) {
		conv_func((const float *) &_in[-(h_vec->len - 1)],
			  (const float *) h_vec->data,
			  (float *) _out, 1);
	} else if (conv_func_n) {
		conv_func_n((const float *) &_in[-(h_vec->len - 1)],
			    (const float *) h_vec->data,
			    (float *) _out, h_vec->len, 1);

	} else {
		conv_real_generic((const float *) &_in[-(h_vec->len - 1)],
				  (const float *) h_vec->data,
				  (float *) _out, h_vec->len, 1);
	}
